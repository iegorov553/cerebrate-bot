...
class TestBaseCallbackHandler:
    ...
    def test_should_force_language_refresh(self, concrete_handler):
        ...
        assert concrete_handler.should_force_language_refresh("language_en") == True
        assert concrete_handler.should_force_language_refresh("menu_language") == True
        assert concrete_handler.should_force_language_refresh("settings_enable") == False
...
class TestSettingsCallbackHandler:
    ...
    def test_can_handle_settings_callbacks(self, handler):
        """Test callback data recognition."""
        assert handler.can_handle("menu_settings") == True
        assert handler.can_handle("settings") == True
        assert handler.can_handle("settings_toggle_notifications") == True
...
class TestFeedbackCallbackHandler:
    ...
    def test_can_handle_feedback_callbacks(self, handler):
        """Test callback data recognition."""
        assert handler.can_handle("menu_feedback") == True
        assert handler.can_handle("feedback_menu") == True
        assert handler.can_handle("feedback_bug_report") == True
        assert handler.can_handle("feedback_feature_request") == True
        assert handler.can_handle("feedback_general") == True
        assert handler.can_handle("settings_view") == False
        assert handler.can_handle("friends_list") == False
    ...
    async def test_start_feedback_session(self, handler):
        ...
        with patch('bot.feedback.FeedbackManager') as mock_feedback_manager, \
             patch('bot.utils.rate_limiter.MultiTierRateLimiter') as mock_rate_limiter:
...
class TestFriendsCallbackHandler:
    ...
    def test_can_handle_friends_callbacks(self, handler):
        """Test callback data recognition."""
        assert handler.can_handle("menu_friends") == True
        assert handler.can_handle("friends") == True
        assert handler.can_handle("friends_list") == True
        assert handler.can_handle("friends_add") == True
        assert handler.can_handle("friends_discover") == True
        assert handler.can_handle("add_friend:123456") == True
        assert handler.can_handle("settings_view") == False
        assert handler.can_handle("feedback_menu") == False
...
class TestAdminCallbackHandler:
    ...
    def test_can_handle_admin_callbacks(self, handler):
        """Test callback data recognition."""
        assert handler.can_handle("menu_admin") == True
        assert handler.can_handle("admin_panel") == True
        assert handler.can_handle("admin_broadcast") == True
        assert handler.can_handle("admin_stats") == True
        assert handler.can_handle("admin_health") == True
        assert handler.can_handle("settings_view") == False
        assert handler.can_handle("friends_list") == False
...
class TestQuestionsCallbackHandler:
    ...
    def test_can_handle_questions_callbacks(self, handler):
        """Test callback data recognition."""
        assert handler.can_handle("menu_questions") == True
        assert handler.can_handle("questions") == True
        assert handler.can_handle("questions_noop") == True
        assert handler.can_handle("questions_create") == True
        assert handler.can_handle("questions_list") == True
        assert handler.can_handle("questions_edit:123") == True
        assert handler.can_handle("questions_delete:456") == True
        assert handler.can_handle("settings_view") == False
        assert handler.can_handle("admin_panel") == False
