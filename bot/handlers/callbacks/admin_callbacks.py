...
class AdminCallbackHandler(BaseCallbackHandler):
    ...
    async def _handle_admin_panel(self, query: CallbackQuery, translator: Translator) -> None:
        ...
        await query.edit_message_text(admin_text, reply_markup=keyboard, parse_mode="Markdown")
    ...
    async def _handle_admin_action(
        ...
        if action == "broadcast":
            ...
            self.logger.warning("Unknown admin action", user_id=user.id, action=action)
            # Fallback to main menu
            await self._handle_back_to_main(query, translator)

    async def _check_admin_access(self, user_id: int, query: CallbackQuery, translator: Translator) -> bool:
        ...
        try:
            ...
            if not admin_ops.is_admin(user_id):
                ...
                await query.edit_message_text(
                    f"{translator.access_denied('admin.access_denied')}" f"{translator.translate('admin.admin_only')}",
            ...
            self.logger.error("Error checking admin access", user_id=user_id, error=str(e))
    ...
    async def _handle_stats(self, query: CallbackQuery, translator: Translator) -> None:
        ...
        try:
            ...
            await query.edit_message_text(translator.translate("admin.loading_stats"), parse_mode="Markdown")
            ...
            stats_text = (
                f"{translator.translate('admin.user_stats')}\n\n"
            ...
            await query.edit_message_text(
                stats_text, reply_markup=KeyboardGenerator.admin_menu(translator), parse_mode="Markdown"
            )
    ...
    async def _handle_health_check(
        ...
        try:
            ...
            # Show loading indicator
            await query.edit_message_text(translator.translate("admin.health_checking"), parse_mode="Markdown")
            ...
            self.logger.info("Health check completed", user_id=user.id, system_status=health_status.status)

        except Exception as e:
            self.logger.error("Error in health check", user_id=user.id, error=str(e))
    ...
    async def _format_health_message(self, health_status, translator: Translator) -> str:
        ...
        for name, component in health_status.components.items():
            ...
            if component.details:
                important_details = {
                    k: v
                    for k, v in component.details.items()
