...
class FeedbackCallbackHandler(BaseCallbackHandler):
    ...
    def can_handle(self, data: str) -> bool:
        ...
        feedback_callbacks = {"menu_feedback", "feedback_menu"}

        return data in feedback_callbacks or data.startswith("feedback_")
    ...
    async def _handle_feedback_menu(self, query: CallbackQuery, translator: Translator) -> None:
            ...
            [
                ...
                [
                    InlineKeyboardButton(
                        translator.translate("feedback.feature_request"), callback_data="feedback_feature_request"
                    )
                ],
    ...
    async def _handle_feedback_action(self, query: CallbackQuery, data: str, translator: Translator) -> None:
        ...
        if data in ["feedback_bug_report", "feedback_feature_request", "feedback_general"]:
            ...
            self.logger.warning("Unknown feedback action", user_id=user.id, action=data)

    async def _start_feedback_session(self, query: CallbackQuery, data: str, translator: Translator) -> None:
        ...
        try:
            ...
            rate_limiter = MultiTierRateLimiter(feedback_rate_limit=self.config.feedback_rate_limit)
            feedback_manager = FeedbackManager(self.config, rate_limiter, self.user_cache)
            ...
            if success:
                ...
                self.logger.info("Feedback session started", user_id=user.id, feedback_type=feedback_type)
            else:
                await self._show_rate_limited(query, translator)

        except Exception as e:
            self.logger.error("Error starting feedback session", user_id=user.id, feedback_type=feedback_type, error=str(e))

            # Show generic error
            await self._show_feedback_error(query, translator)

    async def _show_feedback_disabled(self, query: CallbackQuery, translator: Translator) -> None:
        """Show feedback disabled message."""
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(translator.translate("menu.back"), callback_data="back_main")]])

        await query.edit_message_text(translator.translate("feedback.disabled"), reply_markup=keyboard, parse_mode="Markdown")

        self.logger.debug("Feedback disabled message shown", user_id=query.from_user.id)

    async def _show_rate_limited(self, query: CallbackQuery, translator: Translator) -> None:
        ...
        await query.edit_message_text(
            translator.translate("feedback.rate_limited"), reply_markup=keyboard, parse_mode="Markdown"
        )

        self.logger.debug("Rate limited message shown", user_id=query.from_user.id)

    async def _show_feedback_prompt(self, query: CallbackQuery, feedback_type: str, translator: Translator) -> None:
        ...
        self.logger.debug("Feedback prompt shown", user_id=query.from_user.id, feedback_type=feedback_type)

    async def _show_feedback_error(self, query: CallbackQuery, translator: Translator) -> None:
        """Show feedback error message."""
        keyboard = InlineKeyboardMarkup(
            [[InlineKeyboardButton(translator.translate("menu.back"), callback_data="feedback_menu")]]
        )

        await query.edit_message_text(translator.translate("errors.general"), reply_markup=keyboard, parse_mode="Markdown")

        self.logger.debug("Feedback error message shown", user_id=query.from_user.id)
