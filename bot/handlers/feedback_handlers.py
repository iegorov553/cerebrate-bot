...
async def handle_feedback_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if session.get("status") == "awaiting_description":
            ...
            await handle_feedback_description(update, context, feedback_manager, translator, message_text)
...
async def handle_feedback_description(
    update: Update, context: ContextTypes.DEFAULT_TYPE, feedback_manager: FeedbackManager, translator, description: str
) -> None:
    ...
    session = await feedback_manager.get_feedback_session(user.id)
    if not session:
        await update.message.reply_text(translator.translate("feedback.session_expired"), parse_mode="Markdown")
        return
    ...
    await feedback_manager.user_cache.set(f"feedback_session_{user.id}", session, ttl=3600)
        ...
        [
            [
                InlineKeyboardButton(
                    translator.translate("feedback.confirm_send"), callback_data=f"feedback_confirm_{user.id}"
                ),
                InlineKeyboardButton(
                    translator.translate("feedback.confirm_cancel"), callback_data=f"feedback_cancel_{user.id}"
                ),
            ]
        ]
    )
...
async def handle_feedback_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE, action: str, user_id: int) -> None:
    ...
    if action == "confirm":
        ...
        if not session or "description" not in session:
            await query.edit_message_text(translator.translate("feedback.session_expired"), parse_mode="Markdown")
            return

        description = session["description"]

        # Submit to GitHub
        issue_url = await feedback_manager.submit_feedback(user.id, user.username, description)
