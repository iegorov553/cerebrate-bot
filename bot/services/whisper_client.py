...
class WhisperClient:
    ...
    async def _validate_audio_file(self, file_path: str, duration_seconds: Optional[int] = None) -> None:
        ...
        if duration_seconds and duration_seconds > self.max_duration_seconds:
            raise AudioTooLongError(f"Audio duration {duration_seconds}s exceeds limit {self.max_duration_seconds}s")
            ...
            def _groq_transcribe():
                with open(file_path, "rb") as audio_file:
                    transcription = self.groq_client.audio.transcriptions.create(
                        file=(file_path, audio_file.read()), model=model, language=language
                    )
                    return transcription.text

            # Run in executor with timeout
            text = await asyncio.wait_for(
                asyncio.get_event_loop().run_in_executor(None, _groq_transcribe), timeout=self.groq_timeout
            )
    ...
    async def _transcribe_with_openai(self, file_path: str, language: Optional[str] = None) -> str:
        """
        ...
        try:
            ...
            with open(file_path, "rb") as audio_file:
                transcription_params = {"model": self.openai_model, "file": audio_file, "response_format": "text"}

                if language:
                    transcription_params["language"] = language

                transcription = await asyncio.wait_for(
                    self.openai_client.audio.transcriptions.create(**transcription_params), timeout=self.openai_timeout
                )
    ...
    async def transcribe_audio(
        self, file_path: str, language: Optional[str] = None, duration_seconds: Optional[int] = None, use_cache: bool = True
    ) -> str:
        """
        ...
        try:
            ...
            if self.groq_client:
                try:
                    ...
                    await self._notify_admin(
                        f"⚠️ Groq {self.groq_primary_model} лимит исчерпан, переключаемся на {self.groq_fallback_model}"
                    )
            ...
            if self.groq_client:
                try:
                    ...
                    await self._notify_admin("🔄 Groq лимиты исчерпаны, используем OpenAI")
    ...
    async def get_transcription_stats(self) -> Dict[str, Any]:
        ...
        if self.openai_client:
            providers.update({"openai_model": self.openai_model, "openai_timeout": self.openai_timeout})
