...
class QuestionManager:
    ...
    async def determine_question_for_message(
        self, user_id: int, reply_to_message_id: Optional[int] = None
        ...
        try:
            ...
            if reply_to_message_id:
                notification = await self.question_ops.get_notification_by_message_id(user_id, reply_to_message_id)
    ...
    async def create_custom_question(self, user_id: int, question_data: Dict) -> Optional[Dict]:
        """
    ...
    async def update_question_settings(self, question_id: int, settings: Dict) -> bool:
        """
    ...
    async def update_question_text_with_versioning(self, question_id: int, new_text: str) -> Optional[int]:
        """
    ...
    async def save_notification_for_reply(self, user_id: int, question_id: int, telegram_message_id: int) -> bool:
        """
        ...
        """
        try:
            return await self.question_ops.save_notification(user_id, question_id, telegram_message_id)
    ...
    def _validate_question_settings(self, settings: Dict) -> bool:
        ...
        if "question_name" in settings and (
            len(settings["question_name"].strip()) == 0 or len(settings["question_name"]) > 100
        ):
            logger.warning("Invalid question name length")
            return False
