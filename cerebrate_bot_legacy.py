...
from bot.utils.datetime_utils import safe_parse_datetime as modular_safe_parse_datetime
...
# Legacy supabase client (will be replaced gradually)
supabase: Client = db_client.client

# Legacy cache interface - use the new TTLCache
cache = user_cache

# Legacy CacheManager for backward compatibility with tests
class CacheManager:
    """Legacy wrapper for TTLCache for backward compatibility."""
    
    def __init__(self):
        self._ttl_cache = user_cache
    
    def get(self, key: str, default=None):
        """Get value from cache."""
        # TTLCache is async, but we need sync interface for legacy code
        try:
            import asyncio
            loop = asyncio.get_event_loop()
            if loop.is_running():
                # We're in an async context, create a task
                task = asyncio.create_task(self._ttl_cache.get(key, default))
                return default  # For now, return default in async context
            else:
                return loop.run_until_complete(self._ttl_cache.get(key, default))
        except Exception:
            return default
    ...
    def set(self, key: str, value, timeout_seconds: int = 300):
        ...
        try:
            ...
            if loop.is_running():
                # We're in an async context, create a task
                asyncio.create_task(self._ttl_cache.set(key, value))
            else:
                loop.run_until_complete(self._ttl_cache.set(key, value))
        except Exception:
            pass
    ...
    def invalidate(self, key: str):
        ...
        try:
            ...
            loop = asyncio.get_event_loop()
            if loop.is_running():
                asyncio.create_task(self._ttl_cache.invalidate(key))
            else:
                loop.run_until_complete(self._ttl_cache.invalidate(key))
        except Exception:
            pass
    ...
    def clear(self):
        ...
        try:
            ...
            loop = asyncio.get_event_loop()
            if loop.is_running():
                asyncio.create_task(self._ttl_cache.clear())
            else:
                loop.run_until_complete(self._ttl_cache.clear())
        except Exception:
            pass
